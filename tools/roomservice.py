...
# Copyright (C) 2012/2013 TEAM-NITROUS Project
#
...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from TEAM-NITROUS Github (http://github.com/TEAM-NITROUS)." % device

repositories = []

page = 1
while not depsonly:
    result = json.loads(urllib2.urlopen("https://api.github.com/users/TEAM-NITROUS/repos?page=%d" % page).read())
    if len(result) == 0:
        break
    for res in result:
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/NOSP_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'TEAM-NITROUS/%s already exists' % (repo_name)
            else:
    ...
    f = open('.repo/local_manifests/NOSP_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for TEAM-NITROUS/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: TEAM-NITROUS/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "TEAM-NITROUS/%s" % repo_name, "revision": "lp5.0" })
...
def fetch_dependencies(repo_path):
    ...
    dependencies_path = repo_path + '/NOSP.dependencies'
    syncable_repos = []
...
print "Repository for %s not found in the TEAM-NITROUS Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/NOSP_manifest.xml" % device
